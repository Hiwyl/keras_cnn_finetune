...
def vgg19_all_lr(classes,epochs,steps_per_epoch,validation_steps,input_shape):
    # 加载数据
    ...
    predictions = Dense(256, activation='sigmoid')(predictions)
    if classes==1:
    ...
    asdl = Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
    if classes==1:
    ...
    model.fit_generator(train_batches, steps_per_epoch=steps_per_epoch, validation_data=valid_batches,
                        validation_steps=validation_steps, epochs=20, verbose=2)
    # 再训练全部权重
    for layer in model.layers:
        layer.trainable = True
    asdl = Adam(lr=0.00001, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
    if classes==1:
        model.compile(loss='binary_crossentropy', optimizer=asdl, metrics=['accuracy'])
    else:
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    filepath = "weights/vgg19two_{epoch:04d}.h5"
...
if  __name__=="__main__":
    vgg19_all_lr(1,200,16,4,(224,224))