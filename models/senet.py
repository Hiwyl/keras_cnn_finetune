...
# # config = tf.ConfigProto(intra_op_parallelism_threads=18, inter_op_parallelism_threads=18, device_count={'CPU': 9})
...
class SEResNeXt(object):
    def __init__(self, num_classes=10, size=96, depth=64, reduction_ratio=4, num_split=8, num_block=3):
...
def senet(classes,epochs, steps_per_epoch, validation_steps,input_shape):
    # 加载数据
    train_batches, valid_batches= load_data(input_shape)
    model = SEResNeXt(classes,input_shape[0]).model
    if classes==1:
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    filepath = "../weights/senet_{epoch:04d}.h5"
